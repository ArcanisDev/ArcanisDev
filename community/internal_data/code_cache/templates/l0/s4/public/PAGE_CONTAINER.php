<?php
// FROM HASH: af5ab10158092acae691094bb87ca579
return array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	<li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
		<a href="' . $__templater->escape($__vars['href']) . '" itemprop="item">
			';
	if (($__vars['href'] == $__vars['xf']['homePageUrl']) AND $__templater->func('property', array('dt_homeCrumb_icon', ), false)) {
		$__finalCompiled .= '
...
';
		...
		$__compilerTemp3 .= '
				<div class="p-header-logo p-header-logo--text">
					<a href="' . ((($__vars['xf']['options']['logoLink'] AND $__vars['xf']['homePageUrl'])) ? $__templater->escape($__vars['xf']['homePageUrl']) : $__templater->func('link', array('index', ), true)) . '">
							' . $__templater->func('property', array('dt_text_logo', ), true) . '
					</a>
				</div>
			';
	} else {
	...
	if ($__vars['xf']['visitor']['user_id']) {
		...
		if (($__vars['xf']['visitor']['user_state'] == 'rejected') OR ($__vars['xf']['visitor']['user_state'] == 'disabled')) {
			...
			$__compilerTemp5 .= '
							<a href="' . $__templater->func('link', array('account', ), true) . '"
								class="p-navgroup-link p-navgroup-link--iconic p-navgroup-link--user ' . ((($__vars['xf']['visitor']['conversations_unread'] OR $__vars['xf']['visitor']['alerts_unread'])) ? 'badgeContainer badgeContainer--highlighted' : '') . '"
		...
		if ($__templater->func('property', array('dt_triggers_style', ), false) == 'option2') {
			$__compilerTemp5 .= '
							<a href="' . $__templater->func('link', array('login', ), true) . '" class="p-navgroup-link p-navgroup-link--iconic p-navgroup-link--logIn" data-xf-click="menu">
								<i aria-hidden="true"></i>
		...
		$__compilerTemp5 .= '
						<div class="menu menu--structural menu--medium" data-menu="menu" data-nav-id="userLogin" aria-hidden="true" data-href="' . $__templater->func('link', array('login', ), true) . '"></div>
						';
...
';
	if ($__templater->func('property', array('publicNavSticky', ), false) == 'primary') {
		$__finalCompiled .= '
	<div class="p-navSticky p-navSticky--primary ' . ((($__vars['xf']['visitor']['is_moderator'] AND $__templater->func('property', array('dt_sticky_staff_tools', ), false))) ? 'p-staffSticky' : '') . '" data-xf-init="sticky-header">
...
';
		...
		$__finalCompiled .= '
	<div class="p-navSticky p-navSticky--all ' . ((($__vars['xf']['visitor']['is_moderator'] AND $__templater->func('property', array('dt_sticky_staff_tools', ), false))) ? 'p-staffSticky' : '') . '" data-xf-init="sticky-header">
		' . $__templater->filter($__vars['navHtml'], array(array('raw', array()),), true) . '
		' . $__templater->filter($__vars['subNavHtml'], array(array('raw', array()),), true) . '
	</div>
';
...
<div class="p-body">
			...
			$__finalCompiled .= '
				<div class="p-guest--message">
					<div class="p-guest--message-content">
						' . $__templater->func('property', array('dt_guest_message_content', ), true) . '
					</div>
				</div>
			';
		}
	...
	$__finalCompiled .= '
		...
		'selectedNavEntry' => $__vars['selectedNavEntry'],
			...
			';
	if ($__vars['sidebar']) {
		if ($__templater->func('property', array('dt_collapsible_sidebar', ), false) AND ((!$__templater->func('property', array('dt_disable_sidebar', ), false)) AND $__vars['sidebar'])) {
			$__finalCompiled .= '<span id="collapse-side" class="collapseTrigger collapseTrigger--block ' . ((!$__templater->func('is_toggled', array('_side-1', ), false)) ? ' is-active' : '') . '" data-xf-click="toggle" data-xf-init="toggle-storage" data-storage-type="cookie" data-target=".p-body-sidebar, .p-body-content" data-storage-key="_side-1"></span>';
		}
	}
	$__finalCompiled .= '
		</div>
	...
	$__finalCompiled .= '
			...
			<div class="p-body-content ' . ((!$__templater->func('is_toggled', array('_side-1', ), false)) ? 'is-active' : '') . '">
	...
	if ($__vars['sidebar']) {
		$__finalCompiled .= '
				<div class="p-body-sidebar ' . ((!$__templater->func('is_toggled', array('_side-1', ), false)) ? 'is-active' : '') . '">
					' . $__templater->callAdsMacro('container_sidebar_above', array(), $__vars) . '
					';
		if ((!$__vars['xf']['visitor']['user_id']) AND $__templater->func('property', array('dt_sidebar_login', ), false)) {
			$__finalCompiled .= '
						' . $__templater->includeTemplate('dt_sidebar_login', $__vars) . '	
					';
		}
		$__finalCompiled .= '
					';
...
<footer class="p-footer" id="footer">
	...
	if (strlen(trim($__compilerTemp14)) > 0) {
		$__finalCompiled .= '
			...
			' . $__compilerTemp14 . '
				';
		if (!$__templater->func('property', array('dt_removecopy', ), false)) {
			$__finalCompiled .= '
					<br />
