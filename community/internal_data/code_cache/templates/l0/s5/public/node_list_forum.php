<?php
// FROM HASH: 55f0d1ea2cd1d99e99714765c7240cf8
return array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->func('property', array('dt_node_background', ), false) AND $__vars['node']['node_background']) {
		$__finalCompiled .= '
				<div class="node-img">
					<span class="node-img-inner"><i style="background-image: url(\'' . $__templater->escape($__vars['node']['node_background']) . '\')"></i></span>
				</div>
			';
	} else {
		$__finalCompiled .= '
	...
	if ($__templater->func('property', array('dt_new_indicator', ), false)) {
		$__finalCompiled .= '<span class="node--newIndicator">' . 'New' . '</span>';
	}
	$__finalCompiled .= '
				</h3>
				';
	...
	if (!$__vars['extras']['privateInfo']) {
		$__finalCompiled .= '
						<div class="node-statsMeta">
							';
		if ($__templater->func('property', array('dt_nodestat_simple', ), false)) {
			$__finalCompiled .= '
		...
		if ($__templater->func('property', array('dt_nodestat_simple', ), false)) {
			$__finalCompiled .= '
						<dl class="pairs pairs--rows">
							<dt>' . $__templater->filter($__vars['extras']['discussion_count'], array(array('number_short', array(1, )),), true) . '</dt>
							<dd>' . $__templater->fontAwesome('fa-comment-alt', array(
			)) . '</dd>
						</dl>
					';
		} else {
			$__finalCompiled .= '
						...
						<dl class="pairs pairs--rows">
							<dt>' . $__templater->filter($__vars['extras']['message_count'], array(array('number_short', array(1, )),), true) . '</dt>
							<dd>' . $__templater->fontAwesome('fa-comments-alt', array(
			)) . '</dd>
						</dl>
					';
		}
